...
def genetic_mtr(n, graph, population_size, clasterization1, clust_dist, population1,
    ...
    for iter in tqdm(range(max_iter)):
        ...
        if len(routes) == 1:
            ...
            for clust in range(len(routes)):
                routes_mtrs.append(my_matr(n, routes[clust]))
                ...
                if alp < 0:
                    for k in range(len(routes)):
                        if k != i:
                            for j in range(len(routes[i])):
                                if balans:
                                    distances[j] += (alp * balans_dist * my_distance(routes[i][j],
                                                                                    routes_mtrs[k])) / balans_alp
                                else:
                                    distances[j] += (alp * my_distance(routes[i][j],
                                                                      routes_mtrs[k])) / balans_alp
                    ...
                    for k in range(len(routes)):
                        if (clust_hunter[k] <= hunter_prob) & (k != i):
                            for j in range(len(routes[i])):
                                if balans:
                                    distances[j] += (bet * balans_dist * my_distance(routes[i][j],
                                                                                    routes_mtrs[k])) / balans_alp
                                else:
                                    distances[j] += (bet * my_distance(routes[i][j],
                                                                      routes_mtrs[k])) / balans_alp
                else:
                    for k in range(len(routes)):
                        if (clust_hunter[k] > hunter_prob) & (k != i):
