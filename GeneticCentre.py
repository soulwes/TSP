...
def genetic_centre(n, graph, population_size, clasterization1, clust_dist, population1,
    ...
    for iter in tqdm(range(max_iter)):
        ...
        if len(routes) == 1:
            ...
            routes_centres = []
            for clust in range(len(routes)):
                routes_centres.append(my_centre(routes_sets[clust]))
            sum_dist = 0
            pop_siz = 0
            balans_alp = 0
            balans_bet = 0
                ...
                if alp < 0:
                    for k in range(len(routes)):
                        if k != i:
                            for j in range(len(routes[i])):
                                if balans:
                                    distances[j] += (alp * balans_dist * jaccard_distance_indiv(routes_sets[i][j],
                                                                                    routes_centres[k])) / balans_alp
                                else:
                                    distances[j] += (alp * jaccard_distance_indiv(routes_sets[i][j],
                                                                      routes_centres[k])) / balans_alp
                    ...
                    for k in range(len(routes)):
                        if (clust_hunter[k] <= hunter_prob) & (k != i):
                            for j in range(len(routes[i])):
                                if balans:
                                    distances[j] += (bet * balans_dist * jaccard_distance_indiv(routes_sets[i][j],
                                                                                    routes_centres[k])) / balans_alp
                                else:
                                    distances[j] += (bet * jaccard_distance_indiv(routes_sets[i][j],
                                                                      routes_centres[k])) / balans_alp
                else:
                    for k in range(len(routes)):
                        if (clust_hunter[k] > hunter_prob) & (k != i):
